...
void handler(int num) 
{
  ...
  printf(" your control c wont effect me please type ( bye )to exit me! or any other instruction to ontinue \n");
	...
	while (1) {
		...
		if(thereis) {
			...
			if(fork() ==0) {
				...
				if(fork() == 0 ) {
					printf(" trying | to use | pipes : %s %s %s  \n", args[0],args[1], args[2]);
					close(pipefd[WRITE]);
					dup2(pipefd[READ],0);
					close (pipefd [READ]) ;
			...
			if(strcmp(args[0],"cd") == 0) {
				...
				if(fork() == 0) {
					...
					if(execvp(args[0],args) == -1) {	
						//printf(" ps hell use valid comman please  \n");
						exit(0);
					}
